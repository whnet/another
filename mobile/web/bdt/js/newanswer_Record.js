//偷听方式 answer_way_0 语音  answer_way_1 图文
//   发送回答 580
...
// var voiceprice=$("#askPrice").val();
// var wordprice=$("#WordaskPrice").val();
...
function initVoice(){
    ...
    // 是否偷听点击事件，回答页面中展示 是否付费偷听及同听价格
    $('.appui_switch').on('click',function(e) {
        var price=$("#askPrice").val();
        //listentype 0 关闭 免费 1 打开 收费
        if($('.appui_switch').hasClass('appui_switch-on')){
            listentype=1;
            $('.appui_switch').removeClass("appui_switch-on");
            $("#askPrice").val(price);
        }else{
            listentype=0;
            price=0;
            $('.appui_switch').addClass("appui_switch-on");
        }
    ...
    $('.send-answer').unbind('click').click(function(e) {
        ...
        if($('.appui_switch').hasClass('appui_switch-on')&&price==""){
            listentype=1;
            dataLoadedError("请输入你的偷听价格");
            return;
        }else{
            listentype=0;
            $("#askPrice").val(price);
