...
function initVoice(){
    ...
    // 是否允许追问追答
    $('.appui_switch1').on('click', function (e) {
        var nums=$("#askNums").val();
        if($('.appui_switch1').hasClass('appui_switch-on')){
            zhuitype = 1;
            $('.appui_switch1').removeClass("appui_switch-on");
            $("#askNums").val(nums);
            return;
        }else if(!$('.appui_switch1').hasClass('appui_switch-on')&&nums!="") {
            $('.appui_switch1').addClass("appui_switch-on");
            zhuitype = 1;
        }else{
            $('.appui_switch1').addClass("appui_switch-on");
            zhuitype = 1;
...
//选择回答方式
//
// function chooseAnsWay(index){
//     //  0语音回答 1文字回答
//     $(".answerway"+index).show().siblings().hide();
//     if($(".appui_switch"+index).hasClass("appui_switch-on")){
//         listentype=1;
//         price=$("#askPrice"+index).val();
//     }else{
//         listentype=0;
//         price=0;
//     }
