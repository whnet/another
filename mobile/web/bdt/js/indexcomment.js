...
//评论
        $("#page").on("click",'#comment'+result.data.list[i].id,function(){
            pubComment(this,1);
        })

        //隐藏评论
        $("#page").on("click",'#comment_num_'+result.data.list[i].id,function(){
            showComment(this);
        })
...
// 评论
function pubComment(obj,n){
    var commentBox="";
    // $(obj).parents(".comment_list").siblings(".comment-box").show();
    if(n==1){
        commentBox+='<div class="comment-box">'+
            '<div class="comment-content" contenteditable="true"></div>'+
            '<span class="comment-btn" onclick="pubCommentClick(this,1)">评论1</span>'+
            '</div>';
        $(obj).parents(".time-statistic").after(commentBox);

    }else if(n==2){
        commentBox+='<div class="comment-box">'+
            '<div class="comment-content" contenteditable="true"></div>'+
            '<span class="comment-btn" onclick="pubCommentClick (this,2)">评论2</span>'+
            '</div>';
        $(obj).parents(".comment_list").siblings(".time-statistic").after(commentBox);

        // $(obj).parents(".comment_list").siblings(".comment-box").after(commentBox);
    };
    //禁用评论的点击按钮
...
// 发表评论
function pubCommentClick(text,m){
    // alert(m)
    var Comment="";
    var content=$(".comment-content").html();
    //启用评论框
    $(text).parents("comment-box").find(".comment").css("pointer-events","auto");
    // $(text).parents("focut-list").find(".replay").css("pointer-events","auto");
    if(m==1){
        Comment+="<div class='comment_list'><div class='comment-item' onclick='pubComment(this,2)'>"+
            "<label class='username  fc-blue'>"+"username"+"</label><i>:</i><span class='content'>"+content+"</span>"+
            "</div>" +
            "</div>";
        if(content===""){
            dataLoadedError("评论内容不能为空！")
        }else{
            $(text).parents(".comment-box").siblings(".time-statistic").after(Comment);
            $(".comment-box").hide();
        }

    }else if(m==2){
        Comment+="<div class='comment-item' onclick='pubComment(this,2)'>"+
            "<label ><span class='username  fc-blue'>"+"username2"+"</span>"+"回复"+"<span class='username  fc-blue'>"+"username"+"</span></label><i>:</i><span class='content'>"+content+"</span>"+
            "</div>";
        if(content===""){
            dataLoadedError("评论内容不能为空！")
        }else{
            $(text).parents(".comment-box").siblings(".comment_list").append(Comment);
            $(".comment-box").hide();
        }
    }
...
function showComment(obj){
    ...
    var commentNum = $(obj).siblingd(".comment").text();
    if (commentNum==0) {
        $(obj).css("pointer-events","none");
    }else {
        $(obj).css("pointer-events","auto");
        $(obj).parents(".time-statistic").find(".comment_list").toggleClass("active");
        $(obj).parents(".e-answer-list").find(".comment-list").toggleClass("active");
        // $(obj).find("img").toggleClass("ak_on");
        $(".time-statistic .statistic>a.comment_num").css({"background":"url(../images/up_more.png) left center no-repeat)"})
    }
