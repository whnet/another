...
function pubCommentClick(text,m){
    ...
    if(m==1){
        ...
        if(content===""){
            ...
            $(text).parents(".comment-box").find(".comment_list").append(Comment);
            $(".comment-box").hide();
        }
    }
...
function showComment(obj){
    ...
    var commentNum = $(obj).siblings(".comment").find(".comment-num").text();
    if (commentNum==0) {
        $(obj).css("pointer-events","none");
    }else {
        $(obj).css("pointer-events","auto");
        $(obj).parents(".focus-list").find(".comment-list").toggleClass("active");
